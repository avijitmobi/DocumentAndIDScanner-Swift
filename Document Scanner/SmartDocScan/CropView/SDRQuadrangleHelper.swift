...
//This class upadte four corner properly. If you swap every thing then also it will update
public class SDRQuadrangleHelper {
    ...
    static internal func orderPointsInQuadrangle(quad: [CGPoint]) throws -> [CGPoint] {
        ...
        func findTopLeftPointIndex(quad: [CGPoint]) throws -> Int {
            ...
            guard topLeftPointIdx != -1 else { throw SDRCropError.unknown }
            return topLeftPointIdx
        }
        
        guard quad.count == 4 else { throw SDRCropError.invalidNumberOfCorners }
        guard checkConvex(corners: quad) else { throw SDRCropError.nonConvexRect }
    ...
    //Get crop image from here from the 4 corner point
    static public func cropImage(with image: UIImage, quad: [CGPoint]) throws -> UIImage {
        ...
        guard let transform = perspectiveCorrection else { throw SDRCropError.unknown }
        ...
        guard let perspectiveCorrectedImg = transform.outputImage, let cgImage = context.createCGImage(perspectiveCorrectedImg, from: perspectiveCorrectedImg.extent) else { throw SDRCropError.unknown }
